...
func HandleCmdAppStart(rw http.ResponseWriter, req *http.Request) {
	serverutil.SetHeaders(rw, req, "text/plain")
	name := strings.Replace(req.RequestURI, urls.CmdAppStartUrl, "", 1)
	...
	prj.ScaleItemsDefault(rw, config.GetNamespaceOrDefault(), restConf)
}
