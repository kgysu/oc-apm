...
func HandleCmdAppStop(rw http.ResponseWriter, req *http.Request) {
	serverutil.SetHeaders(rw, req, "text/plain")
	name := strings.Replace(req.RequestURI, urls.CmdAppStopUrl, "", 1)
	...
	prj.ScaleItems(0, rw, config.GetNamespaceOrDefault(), restConf)
}
